...
int readRegister(int fd, int addr, uint8_t * buf, uint8_t len) {
	uint8_t i;
	for (i = 0; i < len; i++) {
		buf[i] = wiringPiI2CReadReg8(fd, addr + i);
	}
	return i;
}

int writeRegister(int fd, int addr, int val) {
...
int main() {
	...
	int fd = wiringPiI2CSetup(BME280_I2C_ADDR_PRIM);
	//printf("fd %d\n", fd);
	if (fd < 0) return -1;
